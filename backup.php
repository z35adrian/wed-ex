...
// ==========================================
// 2. PROSES SIMPAN DATA RSVP
// ==========================================
$status_pesan = "";

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Ambil data dari form dan bersihkan (mencegah SQL Injection dasar)
    ...
    if ($kehadiran === 'Tidak Hadir') {
        $valid = $valid && !empty($pesan);
    }

    if ($valid) {
        $stmt = $conn->prepare("INSERT INTO rsvps (link_id, nama, keluarga, kehadiran, jumlah_tamu, telp, pesan, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
        $stmt->bind_param("issssss", $link_id, $nama, $keluarga, $kehadiran, $jumlah_tamu, $telp, $pesan);
...
<head>
  ...
  <style>
    /* Utility tambahan untuk form PHP */
...
<body class="no-scroll">
  ...
  <main class="invitation-container">
    ...
    <!-- Section 8: RSVP / KONFIRMASI KEHADIRAN (PARALLAX + PHP) -->
    <section id="rsvp" class="card parallax-section parallax-bg-8 form-section">
      <div class="form-overlay">
        <div class="form-overlay-content">
          ...
          <!-- Form RSVP -->
          <form action="wed1.php#rsvp" method="POST">
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      ...
      // Skenario khusus jika halaman di-reload setelah Submit RSVP (Form dikirim)
